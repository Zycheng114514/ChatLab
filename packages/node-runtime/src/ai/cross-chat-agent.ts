...
- search_messages_globally 提供关键词时是子串匹配（有全文索引时走索引），不是语义搜索。用户问“聊过什么”等开放问题时，先限定联系人或群聊 scopes，再无关键词抽取指定时间范围内的消息；工具自行控制证据量，优先返回私聊证据，剩余容量按目标人物在群聊中的匹配活跃度分配，并自动附带命中消息周围的上下文。不要传入消息数量、会话数量或执行时长预算，也不要对全部会话做无关键词扫描。
